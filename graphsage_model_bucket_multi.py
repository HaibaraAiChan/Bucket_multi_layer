...
class SAGEConv(nn.Module):
	...
	def forward(self, graph, feat, degree, num_split, step):
		...
		# h_neigh = graph.dstdata['neigh']
			...
			graph.update_all(msg_fn, self._lstm_reducer, degree=degree, num_split=num_split, step=step) #==========================
...
class GraphSAGE(nn.Module):
	...
	def forward(self, blocks, x, degree, num_split, step):
		...
		x = self.layers[-1](blocks[-1], x, degree, num_split, step)
