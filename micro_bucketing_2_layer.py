...
def  group_degrees_buckets(degrees_group, num_split,step, parameters):
	sorted_val, idx,local_nid_2_global, blocks, model,batch_inputs, batch_labels,  loss_fcn, device, args = parameters
...
def run(args, device, data):
	...
	for run in range(args.num_runs):
		...
		for epoch in range(args.num_epochs):
			...
			num_split = -1
			step = -1
			for _, (input_nodes, seeds, blocks) in enumerate(full_batch_dataloader):
			
				full_batch_inputs, full_batch_labels = load_block_subtensor(nfeats, labels, blocks, device,args)#------------*
				## get the degrees in current layer block
				for layer_id in range(len(blocks)):
					degrees = blocks[layer_id].in_degrees() # local nid as index for degree
				...
				parameters = sorted_val, idx, local_nid_2_global, blocks,model, full_batch_inputs, full_batch_labels,  loss_fcn,device, args
				loss_sum = []
